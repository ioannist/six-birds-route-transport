...
from sixbirds_event.benchmarks.epistemic_six_state import (
    BENCHMARK_ID,
...
def test_benchmark_bundle_runner_completes_end_to_end(tmp_path: Path) -> None:
    ...
    assert index["extendable_under_current_admissibility_semantics"] is True
    assert set(index["sub_runs"]) == {
        "structural",
        "stat_clean",
        "stat_noisy",
        "ccd_clean",
    ...
    assert "Under the current hard-only structural admissibility semantics" in note_text
    assert "extendable" in note_text
    assert "RM is diagnostic-only" in note_text
    ...
    assert "extendable" in result_note.interpretation
    assert "RM remains diagnostic-only" in result_note.interpretation
...
def test_cli_benchmark_runner_works(tmp_path: Path) -> None:
    ...
    assert "extendable_under_current_admissibility_semantics=" in result.stdout
