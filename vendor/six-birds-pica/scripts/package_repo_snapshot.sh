...
def _is_hard_excluded(parts: tuple[str, ...]) -> bool:
    if not parts:
        return False
    if parts[0] == "research":
        return True
...
def _is_allowed(path: pathlib.Path) -> bool:
    ...
    if not rel.parts:
        return False
    if _is_hard_excluded(rel.parts):
        return False
    top = rel.parts[0]
...
for base in allowed_roots:
    if not base.exists():
        continue
    try:
        if _is_hard_excluded(base.relative_to(root).parts):
            continue
    except ValueError:
        continue
    for dirpath, dirnames, filenames in os.walk(base):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in excluded_dirs and not _is_hard_excluded(rel_dir.parts + (d,))
        ]
...
for name in allowed_root_files:
    path = root / name
    rel = path.relative_to(root) if path.exists() else None
    if (
        path.exists()
        and rel is not None
        and rel.parts
        and not _is_hard_excluded(rel.parts)
        and not _is_ignored(path, rules)
    ):
        files.append(path)
