...
class PicaPackagingSurfaceReportArtifacts:
    run_id: str
    run_dir: str
    summary_path: str
...
def write_pica_packaging_surface_report(
    *,
...
) -> PicaPackagingSurfaceReportArtifacts:
    output_root = Path(root).resolve() if root is not None else None
    ...
    summary_path = run_dir / "pica-packaging-surface-summary.json"
    source_index_path = run_dir / "pica-packaging-source-index.json"
    ...
    source_index_path.write_text(
        json.dumps(resolved_surface.source_index, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    summary_payload = resolved_surface.surface.model_dump(mode="json")
    summary_payload["artifact_refs"] = {
        **summary_payload.get("artifact_refs", {}),
        **output_artifacts,
    }
    ...
    result_note = ResultNote(
        ...
        run_id=run_id,
        instance_ids=[resolved_surface.surface.export_bundle_id],
        metrics={
            "distinct_packaging_operator_count": (
                resolved_surface.surface.distinct_packaging_operator_count
            ),
            "distinct_packaging_family_count": (
                resolved_surface.surface.distinct_packaging_family_count
            ),
            "support_slice_count": resolved_surface.surface.support_slice_count,
            "packaging_source_count": len(resolved_surface.surface.source_counts),
        },
        interpretation=(
            "PICA packaging-surface inspection resolved bridge-level producer, operator, family, and selection metadata for packaging-axis comparisons."
        ),
        caveats=[
            "The packaging surface is bridge-derived from observable-first export artifacts and configured selector metadata.",
            "This ticket does not run packaging-axis search logic; it only exposes packaging-surface observability and indexing.",
        ],
        artifact_refs=output_artifacts,
        metadata={
            "analysis_kind": "pica_packaging_surface_inspect",
            "export_bundle_id": resolved_surface.surface.export_bundle_id,
    ...
    manifest = build_run_manifest(
        ...
        or [
            ...
            repo_relative_path(bundle_path, root=source_root),
        ],
        seed=seed,
        input_artifacts={
            "bundle": repo_relative_path(bundle_path, root=source_root),
            **(
                {
                    "package": repo_relative_path(package_path, root=source_root),
                    "provenance": repo_relative_path(provenance_path, root=source_root),
                }
                if package_path is not None and provenance_path is not None
                else {}
            ),
    ...
    return PicaPackagingSurfaceReportArtifacts(
        run_id=run_id,
